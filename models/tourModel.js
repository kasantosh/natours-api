...
const tourSchema = new mongoose.Schema(
  {
    ...
    priceDiscount: {
      ...
      validate: {
        validator: function () {
          //  this only points to current doc on NEW document and doesn't work on update
...
// DOCUMENT MIDDLEWARE - RUNS BEFORE ('pre') bedore .save() and .create()
tourSchema.pre('save', function (next) {
  this.slug = slugify(this.name, { lower: true });
  next();
});

// Embed tour guides from user ids(guides)
// tourSchema.pre('save', async function(next) {
//   const guidesPromises = this.guides.map(async id => await User.findById(id));
//   // above will be an Array pf promises (await User.findById) and hence has to be saved as follows:
