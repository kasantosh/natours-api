...
exports.login = catchAsync(async (req, res, next) => {
    ...
    // 2) Check if the user exists && password is corect
    const user = await (await User.findOne({ email: email }).select('+password'));
    // above method comes from userModel.js
...
exports.protect = catchAsync(async (req, res, next) => {
    // 1) Getting and check if it's there
    let token;
    if (req.headers.authorization && req.headers.authorization.startsWith('Bearer')) {
        token = req.headers.authorization.split(' ')[1];
        // Above header will be sent as authotization: 'Bearer <Token>'
...
exports.updatePassword = catchAsync(async (req, res, next) => {
    ...
    // 3) If password is right, update password
    user.password = req.body.password;
    user.passwordConfirm = req.body.passwordConfirm;
    await user.save();
